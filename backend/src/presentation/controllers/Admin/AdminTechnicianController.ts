...
// Params
import { VerificationQueueFilters } from "../../../domain/repositories/ITechnicianRepository";
// Utils
...
export class AdminTechnicianController {
  ...
  getTechnicianProfile = async (req: Request, res: Response): Promise<Response> => {
    try {
      const { id } = req.params;
      // You might want to log event: ADMIN_GET_TECH_PROFILE_INIT
      ...
      const errorMessage = err instanceof Error ? err.message : String(err);
      // Log event: ADMIN_GET_TECH_PROFILE_FAILED
