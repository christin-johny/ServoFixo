...
import { 
    ...
    type TechnicianProfile // ✅ Import Redux State Type
} from "../../../../store/technicianSlice";
import { useNotification } from "../../../hooks/useNotification";
import ConfirmModal from "../../Admin/Modals/ConfirmModal";
import LoaderFallback from "../../../components/LoaderFallback";

// ✅ Import Repository and DTO
import { 
  getTechnicianProfileStatus, 
  type TechnicianProfileStatusDto 
} from "../../../../infrastructure/repositories/technician/technicianProfileRepository";

// =========================================================
// 1. DATA MAPPING LAYER (Strictly Typed)
// =========================================================
const mapApiDataToProfile = (data: TechnicianProfileStatusDto): TechnicianProfile => {
  return {
    // Identity
    id: data.id,
    ...
    // Defaults for fields not in DTO (required by Redux State)
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                key={item.path}
                onClick={() => navigate(item.path)}
                className={`group flex flex-col items-center justify-center w-full py-3 space-y-1 transition-all duration-200 ${
                  active ? "text-blue-600" : "text-gray-400 hover:text-gray-600"
                }`}
              >
                <div className={`relative p-1 rounded-full transition-all duration-300 ${
                    active ? "bg-blue-50 transform -translate-y-1" : ""
                }`}>
                    <item.icon 
